...
def load(
    ...
    with open(path / "intentsemo.csv", "r") as fd:
        reader = csv.reader(fd)
        
        if (mode == TaskType.TokenClassification):
            X_train_raw, y_train_raw, X, y = load_tokens(reader,include_sentiment,include_emotions, verbose)
        ...
        if (mode == TaskType.SentenceClassification):
            X_train_raw, y_train_raw, X_train, y_train = load_sentences(reader,include_sentiment,include_emotions, True, verbose)
        
        if (mode == TaskType.TextClassification):
            X_train_raw, y_train_raw, X_train, y_train = load_texts(reader,include_sentiment,include_emotions, True, verbose)
