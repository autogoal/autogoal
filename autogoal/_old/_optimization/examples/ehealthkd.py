...
from ..ge import PGE, Grammar, Individual, InvalidPipeline
from ..utils import sdiv, szip
...
class TassGrammar(Grammar):
    ...
    def _score(self, dataset: Dataset):
        ...
        for actual in dev.sentences:
            ...
            for relation in actual.relations:
                match = predicted.find_relation(
                    relation.origin, relation.destination, relation.label
                )
        ...
        msn = (
            correctA + partialA + correctB + incorrectB + missingA + correctC + missingC
        )
        recall = sdiv(top, msn)
    ...
    def _delpunt(self, i, dataset: Dataset):
        ...
        if i.nextbool():
            for sentence in dataset.sentences:
                sentence.text = sentence.text.translate(
    ...
    def _token(self, i, dataset: Dataset):
        ids = (
            max(k.id for sentence in dataset.sentences for k in sentence.keyphrases)
            * 10
        )
    ...
    def _stopw(self, i, dataset: Dataset):
        ...
        for sentence in dataset.sentences:
            sentence.tokens = [
                t for t in sentence.tokens if t.features["norm"] not in sw
            ]
