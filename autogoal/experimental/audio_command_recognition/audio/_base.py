...
class AudioCommandPreprocessor(AlgorithmBase):
    ...
    def __init__(
        ...
        preprocessor: CategoricalValue(
            "AudioVectorToSequence", "MelFrequencyCepstralCoefficients"
        ) = "AudioVectorToSequence",
    ):
    ...
    def _build_preprocessor(self, preprocessor_type):
        ...
        if preprocessor_type == "AudioVectorToSequence":
            ...
            preprocessor = MelFrequencyCepstralCoefficients(
                self._sample_rate, self._n_mfcc
            )
...
class AudioClassifier(AlgorithmBase):
    ...
    def __init__(
        ...
        classifier: algorithm(
            Seq[AudioFeatures], Supervised[VectorCategorical], VectorCategorical
        ),
    ):
    ...
    def run(
        self, data: Seq[AudioFile], labels: Supervised[VectorCategorical]
    ) -> VectorCategorical:
        if self._mode == "train":
