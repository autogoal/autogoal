# AUTOGENERATED ON 2020-01-30 22:20:25.201603
...
class PerceptronTagger(_PerceptronTagger, NltkTrainedTagger):
    ...
    def run(self, input: List(Word())) -> List(Tuple(Word(), Word())):
...
class AffixTagger(NltkTagger):
    def __init__(
        ...
        cutoff: Discrete(min=-100, max=100),
    ):
        self.tagger = _AffixTagger
        self.values = dict(
            affix_length=affix_length, min_stem_length=min_stem_length, cutoff=cutoff
        )

        NltkTagger.__init__(self)

    def run(self, input: List(List(Word()))) -> List(List(Tuple(Word(), Word()))):
...
class BigramTagger(NltkTagger):
    def __init__(self, cutoff: Discrete(min=-100, max=100)):
...
class TnT(_TnT, NltkTrainedTagger):
    def __init__(
        self, Trained: Boolean(), N: Discrete(min=500, max=2000), C: Boolean()
    ):
        NltkTrainedTagger.__init__(self)
