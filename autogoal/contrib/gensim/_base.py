...
class Word2VecEmbeddingSpanish:
    """This class transform a word in embedding vector using Word2Vect of `gensim` (using `Spanish 3B words Word2Vec`).
    ...
    def _load_model(self):
        ...
        return KeyedVectors.load('complete.kv', mmap='r')
    
    @property
    def model(self) -> KeyedVectors:    
        return CacheManager.instance().get('spanish-w2v', self._load_model)

    def run(self, input: Word(domain='general', language='spanish'))-> ContinuousVector():
        """This method use Word2Vect of gensim for tranform a word in embedding vector.
        """
        return self.model.get_vector(input)