...
from .data import is_classifier, is_regressor, is_clusterer, is_transformer, is_algorithm, get_data_for
...
def build_ontology_sklearn(onto):
    ...
    with onto:
        ...
        for cls in imports:
            ...
            for i, part in enumerate(parts[:-1]):
                # print(part)
                the_class = _make_class(part + base_class, (parts[i-1] + base_class) if i > 0 else base_class, classes)
            ...
            with warnings.catch_warnings():
                ...
                for p, meta in sorted(parameters.items(), key=lambda t: t[0]):
                    clss.hasParameter.append(_make_parameter("Sklearn" + cls.__name__, p, meta, onto))
    ...
    CountVectorizerWrapper.hasOutput = get_data_for(onto.Matrix, onto.Continuous, onto.Sparse)
...
def _get_args(cls):
    ...
    drop_args = ['verbose', 'random_state', 'n_jobs', 'max_iter', 'class_weight', 'warm_start', 'copy_X', 'copy_x', 'copy', 'eps']
